...
from config_utils import hm3d_config,mp3d_config, r2r_config,pointnav_config,pointnav_config_mp3d
from socialmap_agent_ronghe import HM3D_Objnav_Agent
import torch
import cv2 
import numpy as np
import quaternion
...
def write_metrics(metrics,path="dynamic_v1_5_0.5.csv"):
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(args.eval_episodes)):
        ...
        evaluation_metrics.append({'success':habitat_agent.metrics['success'],
                                ...
                                'human_collision':habitat_agent.metrics['human_collision'],
                                'psc':habitat_agent.metrics['psc']})
        # exit()
        if habitat_agent.metrics['distance_to_goal']<=0.2:
            success +=1 
        write_metrics(evaluation_metrics,path=f'{args.exp_name}.csv')
        # exit()
